...
class PlannerNode(Node): 
    ...
    def __init__(self): 
        ...
        super().__init__("baseline_planners")
        ...
        self.p_low = p_low
        self.p_high = p_high
        self.l_low = to_logodds(p_low)
        self.l_high = to_logodds(p_high)
    ...
    def lt_callback(self, msg):
        self.lt = msg.data
        # print("left thrust", self.lt)
    def rt_callback(self, msg):
        self.rt = msg.data
        # print("right thrust", self.rt)
    ...
    def coverage_planner(self):
        """ A simple coverage planner that generates waypoints in a lawnmower pattern. """
        
        self.wps.poses = []
        width_between_sweeps = 25  # Define the width between parallel sweeps

        # Retrieve grid size and origin
        ...
        for i in range(0, grid_height, int(width_between_sweeps / self.resolution)):
            x = origin_x + i * self.resolution
            if (i // int(width_between_sweeps / self.resolution)) % 2 == 0:
                # Sweep from left to right
                for j in range(0, grid_width, wp_dist):
                    y = origin_y + j * self.resolution
                    waypoint = Pose()
                    waypoint.position.x = x
                    waypoint.position.y = y
                    self.wps.poses.append(waypoint)
            else:
                # Sweep from right to left
                for j in range(grid_width, 0, -wp_dist):
                    y = origin_y + j * self.resolution
                    waypoint = Pose()
                    waypoint.position.x = x
                    waypoint.position.y = y
    ...
    def random_planner(self):
        ...
        for theta in theta_range:
            ...
            traj, _ = calc_4points_bezier_path(xp, yp, self.psi, xd, yd, psi, offset=3.0, n_points=d)
            trajectories.append(traj)
            
        print("entropy list:", entr, "information gain list:", inf_gains)  
        
        # pick trajectory randomly
        ind = np.random.randint(0, len(trajectories))
        while trajectories[ind] is None:
            ind = np.random.randint(0, len(trajectories))
               
        # print(f"Chosen trajectory index: {ind}, Trajectries: {trajectories}")
        chosen_traj = trajectories[ind]
        
        if chosen_traj is not None:
            t = np.linspace(d/5, d, 5)
            for idx in t:
                wp = Pose()
        ...
        t2 = self.get_time()
        print(f"Time taken for planning: {t2-t1} seconds")

    def greedy_planner(self):
        ...
        if chosen_traj is not None:
            wp = Pose()
            wp.position.x = chosen_traj[1][0]
            wp.position.y = chosen_traj[1][1]
            self.wps.poses.append(wp)
    ...
    def simulate_pose_update(self, traj):
        ...
        # pred_unc_grids = self.predictions(grid)
        inf_gain = 0
        
        x1, y1 = traj[0]
        x2, y2 = traj[1]
        psi = np.arctan2(y1 - self.pos.y, x1 - self.pos.x)
        fov = get_fov(x1, y1, psi)
        og = self.simulate_map_update(x1, y1, psi, fov, og, pred_grids, 1)
        if len(fov) == 0:
            inf_gain += 0
        else:
            # print(self.pred_unc_grids[0][fov[:,:]])
            # inf_gain += np.sum(np.vectorize(self.compute_information_gain)(self.pred_unc_grids[0][fov[:,0], fov[:,1]]))/len(fov)
            ig = 0
            for id in fov:
                d = np.sqrt((id[0] - x1)**2 + (id[1] - y1)**2)
                p = self.pred_unc_grids[0][id[0], id[1]]
                ig += compute_information_gain(p, d)
            inf_gain += ig/ len(fov)

        psi = np.arctan2(y2 - y1, x2 - x1)
        fov = get_fov(x2, y2, psi)
        og = self.simulate_map_update(x2, y2, psi, fov, og, pred_grids, len(pred_grids)-1)
        if len(fov) == 0:
            inf_gain += 0
        else:
            # print(self.pred_unc_grids[i][fov[:,:]])
            # inf_gain += np.sum(np.vectorize(self.compute_information_gain)(self.pred_unc_grids[i][fov[:,0], fov[:,1]]))/len(fov)
            ig = 0
            for id in fov:
                d = np.sqrt((id[0] - x2)**2 + (id[1] - y2)**2)
                p = self.pred_unc_grids[len(pred_grids)-1][id[0], id[1]]
                ig += compute_information_gain(p, d)
            inf_gain += ig/ len(fov)
        ...
        # print(f"Time taken for forwardsimulation: {t3-t2} seconds")
    ...
    def pred_map(self, grid, t, dt=1):
        ...
        for i in np.arange(dt,t,dt):
            ...
            dx = wind_factor * (self.wind_speed + np.random.normal(0, 0.1 * self.wind_speed)) * np.cos(self.wind_dir) * (dt) 
            dy = wind_factor * (self.wind_speed + np.random.normal(0, 0.1 * self.wind_speed)) * np.sin(self.wind_dir) * (dt)
            Rx += dx
            Ry += dy
            shift_x = np.round(Rx)
            shift_y = np.round(Ry)
            ...
            grid = shift(grid, shift=(shift_y, shift_x), order=1, mode='constant', cval=0)
    ...
    def wp_callback(self):
        ...
        self.desired_speed_publisher.publish(Float64(data=self.desired_speed))
