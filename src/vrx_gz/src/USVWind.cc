...
public:
  double varVel = 0;
/// \brief Variable wind direction component

public:
  double varDirection = 0; // Variable wind direction component
  double windMeanDirection = 0; // Mean wind direction (angle in radians)
  double directionGainConstant = 0;
  double directionFilterGain = 0;
  double directionTimeConstant = 0.1; // Time constant for variable wind direction
                        ...
                        sim::EventManager & /*_eventMgr*/)
{
  ...
  if (sdf->HasElement("wind_mean_direction"))
  {
    this->dataPtr->windMeanDirection = sdf->Get<double>("wind_mean_direction");
  }

  if (sdf->HasElement("var_direction_gain_constants"))
  {
    this->dataPtr->directionGainConstant = sdf->Get<double>("var_direction_gain_constants");
  }

  if (sdf->HasElement("var_direction_time_constants"))
  {
    this->dataPtr->directionTimeConstant = sdf->Get<double>("var_direction_time_constants");
  }

  this->dataPtr->directionFilterGain = this->dataPtr->directionGainConstant *
      sqrt(2.0 * this->dataPtr->directionTimeConstant);
  // 
    ...
    sim::EntityComponentManager &_ecm)
{
  ...
  double randomDistDir = dist(*this->dataPtr->randGenerator);
  ...
  // Variable wind direction (new)
  this->dataPtr->varDirection += 1.0 / this->dataPtr->directionTimeConstant *
      (-1.0 * this->dataPtr->varDirection +
      this->dataPtr->directionFilterGain / sqrt(dT.count()) *
      randomDistDir) * dT.count();

  // Current wind direction
  double windDirectionAngle = this->dataPtr->varDirection + this->dataPtr->windMeanDirection;
  // Update wind direction vector
  this->dataPtr->windDirection.X(cos(windDirectionAngle * M_PI / 180));
  this->dataPtr->windDirection.Y(sin(windDirectionAngle * M_PI / 180));
  this->dataPtr->windDirection.Z(0);

  this->dataPtr->windDirMsg.set_data(windDirectionAngle);
